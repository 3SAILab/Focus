...
- **AI 创意工坊**: 文本生成图片、图生图、多图批量生成（1-4张）
...
### 修复
- **多图生成竞态条件**: 废弃队列机制，改用 tempId 显式关联请求和响应，解决并发请求时 UI 卡片错乱问题
- **布局偏移问题**: 批次任务始终使用网格模式渲染，未加载的图片位置显示占位符，避免大图->网格的视觉跳变
- **后端日志增强**: 添加 panic 恢复和详细的 API 请求/响应日志，便于排查问题

### 改进
- 支持同时发送多个生成请求，无需等待前一个完成
- SSE 流式返回多图生成进度，实时显示每张图片的生成状态
...
- 注意：不支持 Windows XP/7/8 和 32-bit 系统

### macOS
- macOS 10.15+ (Catalina 及以上)
- 支持 Intel (x64) 和 Apple Silicon (arm64)
- 安装新版本会覆盖旧版本，用户数据保留在 `~/Library/Application Support/Focus/`
